const inputString = `qFdBBvtHHfvRlfvsqldvqjPpQmnQmjnjjjTRTLGRNG
...
console.log(getMisplacedItemsPioritySum(inputString))
...
function findGroupBadge(backpacks) {
    const backpacksTypes = backpacks.map((backpack) => Array.from(new Set([...backpack])))

    const prunedBackpacksInventory = backpacksTypes.flat()

    const occurenceCountMap = {}

    for (let item of prunedBackpacksInventory) {
        occurenceCountMap[item] = (occurenceCountMap[item] || 0) + 1
        if (occurenceCountMap[item] === backpacks.length) return item
    }
}

function getBadgePrioritiesSum(inputString) {
    const backpacks = inputString.split('\n')
    const badges = []

    for (let i = 0; i < backpacks.length - 1; i += 3) {
        const groupBackpacks = [
            backpacks[i],
            backpacks[i + 1],
            backpacks[i + 2]
        ]

        badges.push(findGroupBadge(groupBackpacks))
    }

    return getSumOfPriorities(badges)
}

console.log(getBadgePrioritiesSum(inputString))